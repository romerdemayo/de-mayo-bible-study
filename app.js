...
function bibleReferencePattern(){
 ...
 return `(?:${names.join('|')})\\.?\\s+\\d{1,3}(?::\\d{1,3}(?:\\s*[-–—]\\s*\\d{1,3})?)?`;
}
...
function backup(){
 ...
 let hasUndo=!!localStorage.getItem(SNAPSHOT_KEY);
